...
if(isset($subid) && isset($testid)) {
	...
	if (mysql_num_rows($result) == 0) {
		$query1 = "SELECT * FROM mst_question WHERE test_id=".$testid." ORDER BY RAND() LIMIT 25";
		$result1 = mysql_query($query1, $cn) or die(mysql_error());
		while($row = mysql_fetch_row($result1)) {
			$query2 = "INSERT INTO mst_useranswer(sess_id, test_id, que_des, ans1, ans2, ans3, ans4, true_ans, your_ans) values ('".session_id()."', ".$testid.",'$row[2]','$row[3]','$row[4]','$row[5]', '$row[6]','$row[7]','')";
	...
	if($_SESSION["qn"] > $rowCount-1) {
		...
		if ($submit=='Next Question' or $submit=='Get Result') {
			if (!isset($ans)) {
				...
				$query4 = "UPDATE mst_useranswer SET your_ans=$ans";
				...
				if($submit=='Get Result') {
					...
					mysql_query("insert into mst_result(login,test_id,test_date,score) values('$login',$tid,now(),'$percentage')") or die(mysql_error());
